...
const isStyleRule = (rule: CSSRule) => rule instanceof CSSStyleRule;

export function getCSSCustomPropsList(): string[][] {
  const sheets = Array.from(document.styleSheets).filter(isSameDomain);
  const rules = sheets
    .reduce((acc: CSSRule[], sheet) => {
      return [...acc, ...Array.from(sheet.cssRules)];
    }, [])
    .filter(isStyleRule);

  const CSSCustromPropNames = rules
    .map((rule) => Array.from((rule as CSSStyleRule).style))
    .flat()
    .filter((prop) => prop.trimStart().startsWith('--'));

  const styles = getComputedStyle(document.documentElement);
  const CSSCustromPairs = CSSCustromPropNames.map((propName) => [propName, styles.getPropertyValue(propName)]);
  return CSSCustromPairs;
}
